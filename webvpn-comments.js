...
class WebVPN {
  ...
  async proxyRoute (ctx, next) {
    ...
    // 确定当前请求协议 (http/https)
    if (ctx.request.searchParams.get("__webvpn_origin_scheme__")){
        ctx.scheme = ctx.request.searchParams.get("__webvpn_origin_scheme__")
        ctx.request.searchParams.delete("__webvpn_origin_scheme__")
    }
    else {
